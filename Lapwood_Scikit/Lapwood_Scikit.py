...
grid=100 #number of grid points including the boundary
...
def equations(t,y,yp):
    ...
    yp[0:(grid-2)*(grid-2)]=(MomEq[1:size,1:size]).reshape((grid-2)*(grid-2)) #reshape to vector
    yp[(grid-2)*(grid-2):2*(grid-2)*(grid-2)]=  (EnergyEq[1:size,1:size]).reshape((grid-2)*(grid-2))  #reshape to vector
    
    global error
    error=yp
    # return yp #join the vectors to pass to IVP solver
...
list_of_linSols=['spbcgs','spgmr','sptfqmr']
for option in list_of_linSols:
    start=datetime.now()
    solver = ode('cvode', equations,linsolver=option)
    timestamps=np.linspace(0,100,1000)
    solution=solver.solve(timestamps,guess)
    print('Time Taken by ',option,' is: ',datetime.now()-start)
    error=np.sum(error**2.0)
    error=np.sqrt(error)/variable**2.0 #divide by the number of equations
    print('Error at final time from ',option,' is: ', error)
#%%
